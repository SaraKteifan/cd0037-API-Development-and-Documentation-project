...
class TriviaTestCase(unittest.TestCase):
    ...
    """
    TODO
    Write at least one test for each test for successful operation and for expected errors. (Done XXX)
    """
    ...
    def test_play_quiz(self):
        res = self.client().post('/quizzes',json={"previous_questions": [1], "quiz_category": {"id": 1, "type": "Science"}})
        data = json.loads(res.data)
    ...
    def test_add_new_question(self):
        res = self.client().post('/questions', json={"question": "Who painted the Mona Lisa?", "answer":"da Vinci", 'difficulty': 2,
    'category': 2,})
        data = json.loads(res.data)
    ...
    def test_get_question_search_without_results(self):
        res = self.client().post('/questions', json={'searchTerm': 'Acceleration'})
        data = json.loads(res.data)
